...
public class F1Sellers extends Fragment {

    private SearchView searchBar;
    private ImageButton qrScannerButton;
    private RecyclerView recyclerView;

    private SSellerAdapter sellerAdapter;
    ...
    private void updateSellerRecyclerView(){
        ...
        customerRef.whereEqualTo("userType", "business")
                .whereEqualTo("userBusinessStatus", "accepted")
                .get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            ...
                            List<DocumentSnapshot> sellers = task.getResult().getDocuments();
                            Collections.shuffle(sellers);
                            List<DocumentSnapshot> randomSellers;
                            if (sellers.size() > 20) {
                                randomSellers = sellers.subList(0, 20); // Pick the first 20 from the shuffled list
                            } else {
                                randomSellers = sellers; // If less than 20, take the entire list
                            }

                            for (DocumentSnapshot document : randomSellers) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        sellerAdapter = new SSellerAdapter(filteredList, this);
